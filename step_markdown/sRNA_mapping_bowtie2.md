...
Use `--xeq` parameter. The reason why I used this is that CIGAR in sam generated by bowtie2 could change M to =/X for more detailed mapping. Using the detailed CIGAR could help us getting the reads matched perfectly (without any mismatch). The assumption we use here is that reads match perfectly to plant genome could be regarding as arising from plant, and the same for bacteria.
...
```bash
cat plant_rna_ratio_50.csv | perl -n -e 'chomp;
@a = split/,/,$_;
if($a[0] eq "name"){print "$_,num\n";}
else{if(not defined $name){$i = 1;
$name = $a[0];print"$_,$i\n";}
